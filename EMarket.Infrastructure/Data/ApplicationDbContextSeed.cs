...
namespace EMarket.Infrastructure.Data
{
    public class ApplicationDbContextSeed
    {
        ...
        {
            if (!await roleManager.RoleExistsAsync(AuthorizationContants.Roles.ADMINISTRATOR))//database sorgusu gelniceye kadar if in çalışmasını engeller
            {
                await roleManager.CreateAsync(new IdentityRole(AuthorizationContants.Roles.ADMINISTRATOR));//rol tanımladık
            }
            ...
            if (!userManager.Users.Any(x => x.UserName == adminUserName))
            {
                await userManager.CreateAsync(new ApplicationUser
                {
                    ...
                    Email = adminUserName
                }, AuthorizationContants.DEFAULT_PASSWORD);

                var adminUser = await userManager.FindByNameAsync(adminUserName);

                await userManager.AddToRoleAsync(adminUser, AuthorizationContants.Roles.ADMINISTRATOR);
            }
        }
